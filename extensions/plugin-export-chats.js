/**
 * Chat Export Plugin for TypingMind
 * 
 * Comprehensive chat export system that exports all chats or individual chats as markdown
 * files in ZIP archives. Features organized folder structure, YAML front matter with
 * metadata, and sidebar button integration.
 * 
 * @author Unknown
 * @version 1.0.0
 * @created 2024-01-01
 * @updated 2025-01-03
 * @category extension
 * @requires plugin-js-zip.js
 * @compatibility TypingMind Web, macOS, Mobile PWA
 * 
 * Features:
 * - Export all chats as ZIP archive with markdown files
 ...
 * - Automatic JSZip library loading
 * - Support for chat tags, folders, and parameters
 * 
 * Installation:
 * 1. Ensure plugin-js-zip.js is available in the same directory
 * 2. Go to TypingMind → Preferences → Advanced Settings → Extensions
 * 3. Add extension URL or paste code directly
 * 4. Restart TypingMind for changes to take effect
 * 
 * Usage:
 * 1. Click "Export All" to export all chats as ZIP
 * 2. Click "Export Chat" to export current chat as markdown
 * 3. Files are automatically downloaded with sanitized names
 * 
 * Configuration:
 * - JSZip dependency loaded from /plugin-js-zip.js
 * - IndexedDB database: CONFIG.DB_NAME
 * - Object store: 'keyval'
 * - Chat key prefix: 'CHAT_'
 * 
 * Storage:
 * - Reads from IndexedDB CONFIG.DB_NAME database
 * - Accesses localStorage for folders and tags data
 * - No persistent settings stored
 * 
 * @license MIT
...
(async () => {
  ...
  const CONFIG = {
    JSZIP_SCRIPT_PATH: CONFIG.JSZIP_SCRIPT_PATH,
    DB_NAME: CONFIG.DB_NAME,
    OBJECT_STORE: 'keyval',
    CHAT_KEY_PREFIX: 'CHAT_',
    STORAGE_KEYS: {
      folders: 'TM_useFolderList',
      tags: 'TM_useChatUniqeTags'
    },
    ELEMENT_IDS: {
      exportAll: 'workspace-tab-export-all',
      exportChat: 'workspace-tab-export-chat',
  ...
  // Utility functions
  const Utils = {
    log: (message) => console.log(`[ChatExport] ${message}`),
    
    safe: (fn, context = 'unknown') => {
      try {
        return fn();
      } catch (e) {
        console.error(`[ChatExport] Error in ${context}:`, e);
        return null;
      }
    }
  };
  ...
  function loadJSZip() {
    return new Promise((resolve, reject) => {
      ...
      if (!existingScript) {
        ...
        script.src = CONFIG.JSZIP_SCRIPT_PATH;
  ...
  // Function to open IndexedDB with the CONFIG.DB_NAME database
  function openDB() {
    return new Promise((resolve, reject) => {
      const dbName = CONFIG.DB_NAME; // Use CONFIG.DB_NAME as the database name
  ...
  // Expose the helper function on window.tmHelpers - only in development mode
